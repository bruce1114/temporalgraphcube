...
# from excel
prearray_time=[
    [1.00,1.48,1.61,1.65,1.66,1.67,1.66,1.54,1.51,1.53],
    [1.00,1.15,1.17,1.16,1.08,1.06,1.06,1.07,1.06,1.06],
    [1.00,0.92,0.88,0.88,0.88,0.89,0.88,0.88,0.89,0.89],
    [1.00,0.87,0.87,0.86,0.86,0.86,0.86,0.86,0.87,0.86]
]

prearray_size=[
    [1.00,2.84,4.96,7.17,9.40,11.64,13.87,16.11,18.35,20.58],
    [1.00,2.42,3.89,5.36,6.83,8.30,9.77,11.24,12.70,14.17],
    [1.00,2.07,3.14,4.21,5.28,6.35,7.42,8.50,9.57,10.64],
    [1.00,1.99,2.99,3.98,4.97,5.96,6.96,7.95,8.94,9.93]
]

stable_time=[
    [1.00,2.05,3.15,3.14,4.11,4.27,4.27,4.28,4.93,5.35],
    [1.00,1.85,2.48,2.48,3.01,3.09,3.09,3.09,3.46,3.70],
    [1.00,1.56,1.91,1.92,2.21,2.26,2.26,2.26,2.47,2.62],
    [1.00,1.47,1.76,1.76,2.01,2.06,2.06,2.06,2.24,2.36]
]

stable_size=[
    [1.00,3.23,6.77,10.30,15.08,20.05,25.03,30.01,35.84,42.26],
    [1.00,2.98,5.79,8.59,12.11,15.73,19.36,22.98,27.11,31.55],
    [1.00,2.66,4.77,6.88,9.38,11.95,14.51,17.07,19.91,22.93],
    [1.00,2.53,4.42,6.31,8.51,10.76,13.01,15.26,17.73,20.34]
]

stree_time=[
    [1.00,1.26,1.45,1.42,1.62,1.56,1.56,1.55,1.73,1.72],
    [1.00,1.23,1.42,1.37,1.58,1.50,1.55,1.53,1.71,1.63],
    [1.00,1.26,1.41,1.41,1.55,1.52,1.56,1.56,1.69,1.62],
    [1.00,1.26,1.41,1.42,1.55,1.54,1.58,1.57,1.69,1.63]
]

stree_size=[
    [1.00,1.91,3.10,4.02,5.32,6.30,7.29,8.22,9.57,10.65],
    [1.00,1.90,3.01,3.91,5.11,6.03,7.01,7.91,9.16,10.12],
    [1.00,1.93,2.98,3.91,5.01,5.93,6.91,7.83,8.99,9.89],
    [1.00,1.94,2.97,3.90,4.97,5.89,6.88,7.81,8.91,9.81]
]
...
lim=[[0.6,2],[0,6],[0.8,2],[0,25],[0,45],[0,12]]
the_ticks=[
...
for i in range(len(time_name)):
    ...
    plt.ylabel('time use ratio',fontsize=15)
...
for i in range(len(size_name)):
    plt.subplot(2,3,i+4)
    plt.xlabel('batch of edges',fontsize=15)
    plt.ylabel('space use ratio',fontsize=15)
    ...
    plt.ylim(lim[i+3])
    plt.xticks(x,fontsize=14)
    plt.yticks(the_ticks[i+3],[str(x) for x in the_ticks[i+3]],fontsize=14)
...
plt.show()
fig.savefig('./index_update.pdf',bbox_inches='tight', format='pdf') 
plt.close()
