...
name=['MQPCL-B','MQPCL-B+','MQPCL-E','MQPCL-E+']
...
data = [dblp, imdb,hospital,lkml,enron]
# dName = ['web-BerkStan','com-LiveJournal']
dName = ['DBLP','IMDB','Hospital','LKML','Enron']
patterns = ['.', 'x', '\\', '//']#, 'O', '.']
fz = 23
...
# plt.legend(ncol=2)
plt.legend(ncol=1, prop={"size":fz+3,"weight":'bold'})
...
fig = plt.gcf()
plt.show()
fig.savefig('./mqpclique_time_default.pdf', bbox_inches='tight', format='pdf') 
plt.close()
# 参数说明：
