...
%		\item For any $S'_i$ in $\Sr'$ and each $(u',v',a')$ in $S'_i$, there exists a nonempty maximum edge set $E=\{(u,v,a)|(u,v,a) \in S_i, u \in [u'] \wedge v \in [v']\; or\; u \in [v'] \wedge v \in [u']\}$, $a'$ is the aggregation result of numeric attributes ($a$) of edges in $E$ obtained by aggregation function specified by user.
		\item $\forall u',v' \in \Vr'$ where $u' \leq v'$ and \revision{any $\Sr[i]$ for $ i \in [1,t_{end}]$}, if there exists a nonempty maximum edge set $E=\{(u,v,a)|(u,v,a) \in \Sr[i],(u \in [u'] \wedge v \in [v']) \vee (u \in [v'] \wedge v \in [u']) \}$, then $ \exists (u',v',a') \in \Sr'[i] $ where $a'$ is the aggregation result of numeric attributes of edges in $E$ obtained by aggregation function upon edges specified by user.
	\end{enumerate}
\end{definition}
...
\begin{definition}[Summarized Snapshot]
	\label{def:sum_snap}
	Given a temporal multidimensional network $\Gr=(\Vr,\Sr,A,W)$, the summarized snapshot $ S $ of $ \Gr $ is a special snapshot of $ \Gr $ \revision{and it is the network structure in the result of a temporal cuboid query $ Q=(A,[1+t_{base},t_{end}+t_{base}]) $ on $ \Gr $}.
\end{definition}
...
\begin{definition}[Nearest Common Descendant]
	\label{def:ncd}
	Given two different views $ A_1' $ and $ A_2' $, \revision{$ cd(A_1',A_2') $ is common descendant of $ A_1' $ and $ A_2' $ and it is also a view in temporal graph cube satisfying $ dim(A_1') \cup dim(A_2') \subseteq dim(cd(A_1',A_2')) $. $ ncd(A_1',A_2') $ is the nearest common descendant of $ A_1' $ and $ A_2' $ and it is one of the common descendants satisfying $ dim(ncd(A_1',A_2'))=dim(A_1') \cup dim(A_2') $.}
\end{definition}
\begin{theorem}
...
\begin{definition}[Similarity of Snapshots]
	...
	Given a snapshot array $ snapArr $ starting from $ 1 $ and containing $ n $ snapshots. \revision{Suppose the summarized snapshot of all snapshots in $ snapArr $ is $ S $ and $ E=\sum_{i=1}^{n}|snapArr[i]| $}, the similarity of snapshots in $ snapArr $ is $ s=\frac{E}{|S|} $.
\end{definition}
